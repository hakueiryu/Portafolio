...
<?php
		...
		$sql100="SELECT SUM(total) SumaTotal
                            FROM (SELECT COUNT(*) total FROM proyecto_inversion WHERE estatus = 'Culminado'
                            UNION ALL
                            SELECT COUNT(*) total FROM proyecto_cgec WHERE estatus = 'Culminado'
                            UNION ALL
                            SELECT COUNT(*) total FROM proyecto_planagua WHERE estatus = 'Culminado'
                            UNION ALL
                            SELECT COUNT(*) total FROM proyecto_espeaten WHERE estatus = 'Culminado'
                            UNION ALL
                            SELECT COUNT(*) total FROM proyecto_gobernacion WHERE estatus = 'Culminado'
                            UNION ALL
                            SELECT COUNT(*) total FROM proyecto_ministerio WHERE estatus = 'Culminado'
                            UNION ALL
                            SELECT COUNT(*) total FROM proyecto_envergadura WHERE estatus = 'Culminado'
                            ) t1";
...
<?php
	...
	//La funcion de javascrip va afuera ya que existe algun conflicto con el echo o el php.
		echo "
				...
				<h2 align='center'>Proyectos Culminado.</h2>
					<tr>
						<th scope='col'>Nro</th>
	...
	<br>
	<br>
<center><a href="../reportes/listado_total_culminado.php" target="_blank"><img src="../iconos/imprimir.png"></a></center>
</body>
</html>
<?php
 } else {
